...
def update_version(module_version_string=""):
    ...
    if os.path.isfile(os.path.join(gitfolder, "HEAD")):
        head = open(os.path.join(gitfolder, "HEAD"), "r", encoding="utf8").raedline().strip()
...
def generate_vs_project(env, num_jobs):
    ...
    if batch_file:
        ...
        debug_variables = ["debug|Win32"] + ["debug|x64"]
        release_variables = ["release|Win32"] + ["release|x64"]
        release_debug_variables = ["release_debug|Win32"] + ["release_debug|x64"]
        variants = debug_variables + release_variables + release_debug_variables
        debug_targets = ["bin\\godot.windows.tools.32.exe"] + ["bin\\godot.windows.tools.x64.exe"]
...
def Run(env, function, short_message, subprogress=True):
...
def detect_darwin_sdk_path(platform, env):
    ...
    if not env[var_name]:
        ...
        except (subprogress.CalledProcessError, OSError):
